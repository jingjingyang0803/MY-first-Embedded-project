...
void loop()
{
  ...
  if (buttonState == HIGH) {
    ...
    lcd.setCursor(0, 0);                  // set cursor to first row
    //         01234567890123456789
    lcd.print("                    ");   //clear the first row to print myIP

    lcd.setCursor(0, 0);                 // set cursor to 1st row
    lcd.print("myIP=");
    lcd.print(Ethernet.localIP());      // print IP number from DHCP server again
    delay(1500);
  }
...
void callback() {
  ...
  if (i_time >= 20) {
    ...
    send_MQTT_message("Group_B_Rain_3", rainLevel);
...
void send_MQTT_message(String S_name, double rainLevel)
{
  char bufa[50];

  char str_s_name[20];//  hold The Convert Data
  S_name.toCharArray(str_s_name, S_name.length() + 1);

  char TempString[20];  //  hold The Convert Data
  dtostrf(rainLevel, 8, 2, TempString);


  // create message with header and data
  // sprintf(bufa,"IOTJS={\"S_name\":\"group_name_here\",\"S_value\":%s}",str_signal_value);
  sprintf(bufa, "IOTJS={\"S_name\":\"%s\",\"S_value\":%s}", str_s_name, TempString);
