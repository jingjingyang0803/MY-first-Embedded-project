...
void loop() {
  ...
  if (command == "SPEED") {
    ...
    if (value > 0 && counter_L < abs(steps_per_degree_L * value)) { // turn right
      digitalWrite(Motor_L_dir_pin, Motor_forward);
      analogWrite(Motor_L_pwm_pin, run_speed);
    }
    else if (value < 0 && counter_R < abs(steps_per_degree_R * value)) { // turn left
  ...
  else if (command == "MOVE" && counter_R < abs(steps_per_cm * value)) {
