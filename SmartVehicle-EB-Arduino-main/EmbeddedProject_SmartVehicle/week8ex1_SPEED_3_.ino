...
const float steps_per_degree_R = 62 / 90.0;  //angular unit:degree
const float steps_per_degree_L = 157 / 90.0;  //angular unit:degree
...
void loop() {
  ...
  if (command == "SPEED")           //===============case 1: SPEED:0-255=========
    ...
    if (value > 0 && counter_L < abs(steps_per_degree_L * value))
      turnRight();
    else if (value < 0 && counter_R < abs(steps_per_degree_R * value))
      turnLeft();
    else
      carStop();
  }
    ...
    forward_or_return = value > 0 ? Motor_forward : Motor_return;

    if (counter_R < abs(steps_per_cm * value))
      carRun();
    else
      carStop();
  }
